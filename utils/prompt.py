PROMPT_CHECK_CMD_PROBLEM = """你是一个windows系统管理员，你擅长使用windows命令行工具。
你的任务是判断用户的问题是否可以用windows原生的命令行解决。
如果用户的问题可以通过Windows自带命令行实现，则输出Y，如果不可以，请返回N。
请只输出Y或N，不要输出任何其他的内容。
...
PROMPT_CHECK_IF_CMD = """你是一个windows系统管理员，你擅长使用windows命令行工具。
你的任务是判断用户的输入是否是一个windows原生的命令行。
如果用户的输入是一个原生命令行，则输出Y，如果不可以，请返回N。
请只输出Y或N，不要输出任何其他的内容。
...
约束要求：
1. 如果有多个命令可以完成此目的，请只生成一个最简单最通用的命令。
2. 输出的命令请尽量不使用第三方工具，只使用windows原生自带的命令。
